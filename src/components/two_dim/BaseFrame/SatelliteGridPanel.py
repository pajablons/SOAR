...
class SatelliteGridPanel(wx.Panel):
    ...
    def makeGrid(self, grid):
        ...
        grid.SetColLabelValue(3, 'Targets Seen')
        grid.SetColLabelValue(4, 'Stations Seen')
    ...
    def updateLocations(self, gdf):
        for index, row in gdf.iterrows():
            ...
            if len(row['stations']) > 0:
                self.grid.SetCellValue(row=table_row, col=3, s=str(row['stations']))
            if len(row['targets']) > 0:
                self.grid.SetCellValue(row=table_row, col=4, s=str(row['targets']))
